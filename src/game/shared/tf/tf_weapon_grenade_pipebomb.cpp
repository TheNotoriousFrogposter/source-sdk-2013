																	...
																	int iPipeBombType, float flMultDmg )
{
	...
	if ( pGrenade )
	{
		...
		pGrenade->SetDamage( ( ( ( ff_use_new_grenade.GetBool() && iNoSpin != 2 ) || pGrenade->m_iType != TF_GL_MODE_REGULAR ) ? pGrenade->GetDamage() : TF_WEAPON_GRENADE_XBOX_DAMAGE ) * flMultDmg );
